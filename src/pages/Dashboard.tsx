...
import { Eye, Users, Heart, Plus, Search, Trash2, Edit2, MoreHorizontal } from 'lucide-react';
...
export default function Dashboard() {
  ...
  const [videos, setVideos] = useState<DashboardVideo[]>([
    {
      ...
      thumbnail: "https://images.unsplash.com/photo-1619983081593-e431bd73ea91?ixlib=rb-1.2.1&auto=format&fit=crop&w=100&q=80",
      status: 'Published',
      likes: 367,
      dislikes: 4767,
    ...
    },
    {
      ...
      thumbnail: "https://images.unsplash.com/photo-1614613535308-eb5fbd3d2c17?ixlib=rb-1.2.1&auto=format&fit=crop&w=100&q=80",
      status: 'Unpublished',
      likes: 367,
      dislikes: 4767,
    ...
    },
    {
      ...
      thumbnail: "https://images.unsplash.com/photo-1516280440614-6697288d5d38?ixlib=rb-1.2.1&auto=format&fit=crop&w=100&q=80",
      status: 'Published',
      likes: 367,
      dislikes: 4767,
    ...
    },
    {
      ...
      thumbnail: "https://images.unsplash.com/photo-1493225255756-d9584f8606e9?ixlib=rb-1.2.1&auto=format&fit=crop&w=100&q=80",
      status: 'Published',
      likes: 367,
      dislikes: 4767,
      dateUploaded: "9/4/12"
    },
    {
      ...
      thumbnail: "https://images.unsplash.com/photo-1514525253440-b393452e3383?ixlib=rb-1.2.1&auto=format&fit=crop&w=100&q=80",
      status: 'Unpublished',
      likes: 367,
      dislikes: 4767,
  ...
  const toggleStatus = (id: string) => {
    setVideos(videos.map(v => 
      v.id === id ? { ...v, status: v.status === 'Published' ? 'Unpublished' : 'Published' } : v
    ));
  };
    ...
    <div className="min-h-screen bg-black text-white p-6 md:p-8">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-10">
        <div>
          ...
          <p className="text-gray-400 text-sm">Track, manage and forecast your customers and orders.</p>
        </div>
        <button 
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-gray-800">
              {videos.map((video) => (
                <tr key={video.id} className="group hover:bg-gray-900/50 transition-colors">
                  {/* Toggle Switch Column */}
                  <td className="p-4">
                    <button 
                        onClick={() => toggleStatus(video.id)}
                        className={`w-11 h-6 rounded-full relative transition-colors duration-200 ease-in-out ${video.status === 'Published' ? 'bg-purple-600' : 'bg-gray-600'}`}
                    >
                        <span className={`absolute left-1 top-1 w-4 h-4 bg-white rounded-full transition-transform duration-200 ${video.status === 'Published' ? 'translate-x-5' : 'translate-x-0'}`} />
                    </button>
                  </td>
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-3 min-w-[240px]">
                        <img src={video.thumbnail} alt="" className="w-10 h-10 rounded-full object-cover" />
